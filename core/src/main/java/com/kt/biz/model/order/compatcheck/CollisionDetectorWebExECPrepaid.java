 ...
 */
public class CollisionDetectorWebExECPrepaid implements CollisionDetector {
            ...
            if (OrderUtils.isTimeOverlapping(me, order)) {
                ...
                switch (chargeType) {
                    ...
                    case MONTHLY_PAY_BY_ACTIVEHOSTS:
                        tmp = detectAgainst(getBiz(), (WebExConfMonthlyPayByAH) chargeScheme);
                        break;
    ...
    private CollisionDetectResult detectAgainst(BizWebExConf b, WebExConfMonthlyPayByAH c) {
        if (OrderUtils.isSameSite(c.getSiteName(), getChargeScheme().getSiteName())) {
    ...
    private WebExECPrepaid getChargeScheme() {
        return (WebExECPrepaid) me.getChargeScheme();
    }
