 ...
 */
public class AlgConferenceByPorts extends Alg {

    private static final Logger LOGGER = Logger.getLogger(AlgConferenceByPorts.class);

    private static final int DEFAULT_HOSTS_VOLUME = 999999;

    @Override
    public void start(OrderBean order, WbxPayload initialState) {
        WebExConfMonthlyPayByPorts chargeScheme = getChargeScheme(order);
    ...
    private WebExConfMonthlyPayByPorts getChargeScheme(OrderBean order) {
        AbstractChargeScheme chargeScheme = order.getChargeScheme();
        if (chargeScheme instanceof WebExConfMonthlyPayByPorts) {
            return (WebExConfMonthlyPayByPorts) chargeScheme;
        }
        return null;
    }
